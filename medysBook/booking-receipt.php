...
<body>
        ...
        <div class="col-lg-7">
          ...
          <?php else: ?>
            ...
            <div class="mc-booking-form-card text-center mt-4" style="padding:2rem 1.5rem;">
              ...
              <!-- QR canvas (generated by JS — instant, no external API) -->
              <div style="display:inline-block;border:4px solid #f0e8e6;border-radius:12px;padding:8px;background:#fff;margin-bottom:1.2rem;">
                <canvas id="mc-qr-canvas" style="display:block;border-radius:6px;"></canvas>
              </div>
  ...
  <script>
    ...
    QRCode.toCanvas(document.getElementById('mc-qr-canvas'), MC_RECEIPT_URL, {
      width: 220,
      margin: 2,
    ...
    function downloadQR() {
      QRCode.toDataURL(MC_RECEIPT_URL, { width: 500, margin: 2 }, function(err, url) {
        if (err) return;
